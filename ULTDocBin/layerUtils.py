"""
Layer Utils for ULT Document Binarization
"""
import os
...
# Keras Layers
################################################################################
class ULTMultiWindow(Layer):
    """
    MultiWindow ULT Keras Layer
    
    1. Instead of doing ULT threshold computation for one window size,
    ...
    3. ULT parameters, namely, k and R, can be selected to be
       trainable or not. Detailed meaning of k and R, please refer
       https://scikit-image.org/docs/stable/api/skimage.filters.html#skimage.filters.threshold_ULT
    ...
    def __init__(self, 
        ...
        super(ULTMultiWindow, self).__init__(**kwargs)
    def get_config(self) :
    ...
    def build(self, input_shape):
        ...
        self.k = self.add_weight(name='ULT_k',
        ...
        self.R = self.add_weight(name='ULT_R',
                                shape=(1,self.num_woi,1,1,1),
                                dtype='float32',
                                initializer='ones',
    ...
    def call(self, x):
        ...
        input_x , xk, xr,xk2,xr2 = x
        xr= K.cast(xr, tf.float64)
        xk= K.cast(xk, tf.float64)
        xk = K.expand_dims(xk,axis=-1)
        xr = K.expand_dims(xr,axis=-1)
        
        xr2= K.cast(xr2, tf.float64)
        xk2= K.cast(xk2, tf.float64)
        xk2 = K.expand_dims(xk2,axis=-1)
        xr2 = K.expand_dims(xr2,axis=-1)
        x = K.cast(input_x, tf.float64)
        x_2 = x**2
        E_x = self._compute_for_all_sizes(x)
        E_x2 = self._compute_for_all_sizes(x_2)
        dev_x = K.sqrt(K.maximum(E_x2 - E_x**2, 1e-6))

        T = xk*E_x + xr*dev_x + xk2*dev_x*E_x + xr2
        T = K.cast(T, 'float32')
        return T
