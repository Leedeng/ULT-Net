"""
Data Utils for ULT Document Binarization
"""
...
class DataGenerator :
    ...
    def __getitem__(self, batch_idx) :
        ...
        for i in indices :
            ...
            #if (self.mode == 'training') :
                #if prng.randn() > 0 :
                    #scale_factor = prng.choice([0.75,1.25,1.5])
                    #img = self.rescale(img,scale_factor)
                    #gt = self.rescale(gt,scale_factor)
            ...
            if (self.mode == 'training') :
                ...
                #rotate
                #if prng.randn() > 0 :
                    #angle = prng.choice([90,180,270])
                    #x = self.rotate(x,angle)
                    #y = self.rotate(y,angle)
                #filp
                #if prng.randn() > 0 :  
                    #flipCode = prng.choice([1,0,-1])
                    #x = self.flip(x,flipCode)
                    #y = self.flip(y,flipCode)
            X.append(x)
            Y.append(y)
    ...
    def rotate(self,image,angle,center=None,scale=1.0):
        ...
        return rotated
    def flip (self,image,flipCode=1):
        flip_image = cv2.flip(image,flipCode)
        return flip_image
    def rescale (self,image,scale=1.0):
        h,w = self.output_shape
        rescaled = cv2.resize(image,(0,0),fx=scale,fy=scale)
        return rescaled
