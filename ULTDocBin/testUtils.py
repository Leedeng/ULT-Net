...
import pandas as pd
import numpy as np 
from layerUtils import ULTLayerObjects
...
def prepare_inference(model_filepath, metrics=['TextAcc', 'Acc', 'F1', 'PSNR']) :
    ...
    """
    for m in metrics :
        assert m in ULTLayerObjects.keys(), \
            f"ERROR: unsupported metric {m}, be sure to register it in `ULTLayerObjects`"  
    model = tf.keras.models.load_model(model_filepath,
                                  custom_objects=ULTLayerObjects,
                                  compile=False,
                                 )
    model.compile('adam', loss='hinge', metrics=[ULTLayerObjects[m] for m in metrics])
    return model

def evaluate_on_datasets(ULT_model, test_datasets, dataset_lut, output_dir) :
    """Evaludate the given ULT binarization model on the required dataset
    ...
    for this in test_datasets :
        ...
        # 3. run evaluation
        ret = ULT_model.evaluate_generator(eval_dataset, L, verbose=1)
        # 4. collect results
        res = dict(zip(ULT_model.metrics_names, ret))
        res['Dataset'] = this
        res['#Samples'] = L
        all_res.append(res)
    # 5. save as a data frame
    csv_headers = ['Dataset', '#Samples'] + ULT_model.metrics_names
    df = pd.DataFrame(all_res, columns=csv_headers)
    pd.set_option('display.float_format','{:.4f}'.format)
    print(f"INFO: successfully evaluated model {ULT_model.name}")
    print(df)
    csv_file = os.path.join(output_dir, ULT_model.name + '.csv')
